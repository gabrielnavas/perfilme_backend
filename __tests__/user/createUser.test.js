...
describe('Create a User', () => {
  ...
  test('should return 201 and user created', async () => {
    ...
    const msDate = new Date(userCreated.createdAt).getTime()
    expect(typeof msDate).toBe('number')
    expect(userCreated.userName).toEqual(`${userCreated.name}${userCreated.id}`)
  })
})
